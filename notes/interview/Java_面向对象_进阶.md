...
父类的属性、方法可以被继承。构造器可以被子类调用。父类私有属性在提供了get、set方法后，子类通过get方法使用。
...
同一个类中定义多个相同名字，但是形参列表不同的方法。像上面名字、形参一样，但是返回值不一样构不成方法重载，会造成编译错误，编译器无法确定要调用哪个方法。
...
见名知意。名字一样，参数列表不一样来区分。

### 2.3 super关键字

#### 1. super和this的区别(蚂**服)

没什么联系，把两个关键字各自的特点说清楚。
