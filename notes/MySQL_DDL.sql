...
#练习2：创建一个表employees_blank，实现对employees表的复制，不包括表数据，在where中添加不可实现的条件，选不出来数据即可
...
ALTER TABLE myemp1
ADD phone_number VARCHAR(20) FIRST;# 添加到表中的第一个的位置

ALTER TABLE myemp1
ADD email VARCHAR(45) AFTER emp_name;# 添加到指定字段后

# 3.2 修改一个字段：数据类型、长度、默认值（略），一般不会操作数据类型
ALTER TABLE myemp1
...
ALTER TABLE myemp1
CHANGE email my_email VARCHAR(50);# 在改名字的同时改长度
...
# COMMIT:提交数据。一旦执行COMMIT，则数据就被永久的保存在了数据库中，意味着数据不可以回滚。操作不可撤销，ROLLBACK后数据不可恢复。
...
#	DELETE FROM：一旦执行此操作，表数据可以全部清除（不带WHERE过滤，代表全部数据）。同时，数据是可以实现回滚的。
